...
Клиент                           Сервер
   |                                |
   |--- HTTP GET с заголовком ----->|  "Upgrade: websocket"
   |    "хочу перейти на WS"        |
...
**Файл:** `src/websocket/stream.ts:88-132`
...
Здесь происходит ключевое: Fastify видит флаг `{ websocket: true }` и понимает, что этот роут не для HTTP, а для WebSocket. Вместо обычного HTTP-ответа он "поднимает" WebSocket-соединение.

### 4.2. Что происходит при подключении клиента

Когда dashboard открывает `ws://localhost:3000/stream?deviceId=test-pi`, сервер вызывает обработчик:
...
export function markDeviceSeen(strDeviceId: string): boolean {
  ...
  mapLastSeen.set(strDeviceId, Date.now());  // записали текущее время
  return bWasKnown;  // вернули, было ли устройство уже известно
}
```
...
export function startHeartbeatMonitor(): NodeJS.Timeout {
  const fnCheck = (): void => {
    ...
    for (const [strDeviceId, nLastSeen] of mapLastSeen.entries()) {
      if (nNow - nLastSeen >= objConfig.nDeviceOfflineThresholdMs) {  // 30 сек
        broadcastDeviceOffline(strDeviceId);  // всем: "устройство пропало"
        mapLastSeen.delete(strDeviceId);       // убрали из списка
      }
    }
  };

  return setInterval(fnCheck, objConfig.nHeartbeatCheckMs);  // каждые 5 сек
}
```
...
Хотя клиентский код не входит в этот репозиторий, важно понимать, как он работает:
